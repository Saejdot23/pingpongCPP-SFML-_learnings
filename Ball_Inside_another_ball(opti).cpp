...
Vector2f relativePos = ball3.getPosition() - ball.getPosition();
float distBetween = sqrt(relativePos.x * relativePos.x + relativePos.y * relativePos.y);
float minDistance = ball.getRadius() + ball3.getRadius();

if (distBetween < minDistance && distBetween > 0) {
    float overlap = minDistance - distBetween;
    Vector2f normal = relativePos / distBetween;
    
    ball.move(-normal * (overlap / 2.0f));
    ball3.move(normal * (overlap / 2.0f));

    Vector2f relativeVel = velocity1 - velocity;
    float velocityNormal = (relativeVel.x * normal.x + relativeVel.y * normal.y);

    if (velocityNormal < 0) {
        float impulse = velocityNormal;
        
        velocity.x += impulse * normal.x;
        velocity.y += impulse * normal.y;
        
        velocity1.x -= impulse * normal.x;
        velocity1.y -= impulse * normal.y;
